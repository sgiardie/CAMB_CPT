    ...
    if (State%CP%WantScalars .and. State%CP%WantTensors .and. State%CP%DoLensing) then
        State%CP%lmax_AF = min(State%CP%lmax_AF, State%CP%Max_l, State%CP%Max_l_tensor, State%CLData%lmax_lensed)
        if (State%CP%lmin_AF < State%CP%lmax_AF) check_lminAF = .true.
    else
        if (.not. State%CP%DoLensing) then
            State%CP%lmax_AF = min(State%CP%lmax_AF, State%CP%Max_l, State%CP%Max_l_tensor)
            if (State%CP%lmin_AF < State%CP%lmax_AF) check_lminAF = .true.
        else if (.not. State%CP%WantTensors) then
            State%CP%lmax_AF = min(State%CP%lmax_AF, State%CP%Max_l, State%CLData%lmax_lensed)
            if (State%CP%lmin_AF < State%CP%lmax_AF) check_lminAF = .true.
        else
            State%CP%lmax_AF = min(State%CP%lmax_AF, State%CP%Max_l)
            if (State%CP%lmin_AF < State%CP%lmax_AF) check_lminAF = .true.
        end if
    end if 
    ...
    if (check_lminAF) then
        ...
        allocate(newCltens_E(State%CP%lmin_AF:State%CP%lmax_AF))
        newCltens_E = 0

        allocate(newCltens_TE(State%CP%lmin_AF:State%CP%lmax_AF))
        newCltens_TE = 0

        allocate(newCltens_B(State%CP%lmin_AF:State%CP%lmax_AF))
        newCltens_B = 0            
        ...
        if (State%CP%WantTensors) then
            State%CLData%Cl_tensor(State%CP%lmin_AF:State%CP%lmax_AF, CT_E) = newCltens_E
            State%CLData%Cl_tensor(State%CP%lmin_AF:State%CP%lmax_AF, CT_Cross) = newCltens_TE
            State%CLData%Cl_tensor(State%CP%lmin_AF:State%CP%lmax_AF, CT_B) = newCltens_B
        end if
